// STAHL S9 — the C64 uppercase/graphics character ROM (the "ghost" glyph source for
// the Font-Linse). These are the first 2 KB of the stock C64 character ROM
...
export const ROM_GLYPH_BYTES = 8

/** The 8 raw bytes of the ROM glyph at a given charset slot (screen code 0–255). */
export function romGlyph(slot: number): readonly number[] {
  if (slot < 0 || slot > 255) return [0, 0, 0, 0, 0, 0, 0, 0]
  const o = slot * ROM_GLYPH_BYTES
  return C64_ROM_FONT_UPPER.slice(o, o + ROM_GLYPH_BYTES)
}

/**
 * The ROM glyph as 64 row-major booleans (8×8, true = pixel set), for drawing the
 * Font-Linse ghost overlay (S9.T3). MSB is the leftmost pixel, byte 0 the top row —
 * the same bit order the C64 uses, so the ghost matches the real glyph shape.
 */
export function romGlyphBits(slot: number): boolean[] {
  const bytes = romGlyph(slot)
  const bits = new Array<boolean>(64)
  for (let y = 0; y < 8; y++) {
    const b = bytes[y] ?? 0
    for (let x = 0; x < 8; x++) bits[y * 8 + x] = ((b >> (7 - x)) & 1) === 1
  }
  return bits
}
